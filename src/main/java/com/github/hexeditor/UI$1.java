...
final class UI$1 implements Runnable {
   ...
   public void run() {
      JFrame var1 = new JFrame(1 < this.val$args.length && this.val$args[0].equals("-slave")?"hexeditor.jar currently linked to  " + this.val$args[1]:"hexeditor.jar     https://github.com/javadev/hexeditor     Updated: 2014-07-29", GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration());
      ...
      if(1 < this.val$args.length && this.val$args[0].equals("-slave")) {
         var1.setBounds(var2.x + var2.width - var4, var2.y + (var2.height + var3.top - var3.bottom - var5) >> 1, var4, var5);
      } else {
         var1.setBounds(var2.x + (var2.width + var3.left - var3.right - var4) >> 1, var2.y + (var2.height + var3.top - var3.bottom - var5) >> 1, var4, var5);
      }
