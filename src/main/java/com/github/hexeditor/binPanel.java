...
public class binPanel extends JPanel implements ActionListener, ItemListener, CaretListener, MouseListener {
   ...
   public binPanel(boolean var1, String[] var2) {
      ...
      String[][] var5 = new String[][]{{"File", "Open", "Save as ", "Close file (Q)", "Screen to Png"}, {"Edit", "Select All", "Undo (Z)", "Redo (Y)", "Cut (X)", "Copy", "Paste (V)", "Find", "Insert (before)", "Delete"}, {"View", "Goto", "Toggle position Mark", "Down to next mark", "Up to previous mark ", "Toggle caret ", "Higher fontSize", "Lower fontSize", "Black/White background"}, {"hidden", "Font +", "Font -"}, {"Help", "Toggle help"}};
      ...
      for(int var3 = 0; var3 < var5.length; ++var3) {
         ...
         if(var3 != 0 || !var1) {
            for(int var4 = 1; var4 < var5[var3].length; ++var4) {
               ...
               if(var3 == 1 && (var4 == 1 || var4 == 3 || var4 == 6 || var4 == 7) || var3 == 2 && (var4 == 1 || var4 == 4 || var4 == 5 || var4 == 7)) {
   ...
   public JComponent help() {
      ...
      try {
         var1.eP.getEditorKit().read(UI.class.getResource(UI.class.getResource(var2) == null?"ReadMeEn.htm":var2).openStream(), var1.eP.getDocument(), 0);
         var1.eP.getEditorKit().read(UI.class.getResource("shortKey.htm").openStream(), var1.eP.getDocument(), var1.eP.getDocument().getLength());
   ...
   private JPanel findPanel() {
      ...
      var6.add(this.fJRB);
      this.fJTF[0].setPreferredSize(new Dimension(this.fJCB[0].getPreferredSize().width + this.fJCB[1].getPreferredSize().width + this.fJCB[2].getPreferredSize().width + this.fJCB[3].getPreferredSize().width, this.fJTF[0].getPreferredSize().height));
   ...
   private JPanel status() {
      String[][] var1 = new String[][]{{"BE", "LE "}, {"Binary", "Byte, signed/unsigned    ", "Short (16), signed", "Short (16), unsigned", "Int (32), signed", "Int (32), unsigned", "Long (64), signed", "Long (64), unsigned", "Float (32)", "Double (64)", "DOS-US/OEM-US/cp437", "UTF-8", "UTF-16"}, {"<html>Big-Endian (natural order) or little-Endian (Intel order).", "<html>Conversion rule for the data following the caret (shown here after)."}};
      ...
      if(var21 == 0) {
         ...
         if(var22 < 5 && !var19.startsWith("0x") && !var19.startsWith("Ox") && !var19.startsWith("ox")) {
            ...
            if(var21 == 0) {
               ...
               this.useFindChar = var1 = 0 != (var32 = var3.compareTo(new BigDecimal(var22 == 3?(double)var13:var9)));
               var15 = var17[0] = var22 == 3?(long)Float.floatToRawIntBits(var13):Double.doubleToRawLongBits(var9);
               if(this.fJCB[0].getSelectedIndex() == 0) {
                  for(var23 = 0; var23 < this.finByte.length; ++var23) {
                     this.finByte[this.finByte.length - var23 - 1] = (byte)((int)(var15 & 255L));
                     var15 >>>= 8;
               ...
               if(this.useFindChar) {
                  var17[1] = var22 == 3?(long)Float.floatToRawIntBits(var14 = 0 < var32?math.nextUp(var13):math.nextDown(var13)):Double.doubleToRawLongBits(var11 = 0 < var32?math.nextUp(var9):math.nextDown(var9));
                  if(var22 == 3) {
                     var18.append(var32 < 0?"&lt; ":"&gt; ").append(this.fForm.format(new BigDecimal((double)var13))).append("<br>" + (var32 < 0?"&gt; ":"&lt; ")).append(this.fForm.format(new BigDecimal((double)var14)));
                  } else {
                     var18.append(var32 < 0?"&lt; ":"&gt; ").append(this.dForm.format(new BigDecimal(var9))).append("<br>" + (var32 < 0?"&gt; ":"&lt; ")).append(this.dForm.format(new BigDecimal(var11)));
                  }
